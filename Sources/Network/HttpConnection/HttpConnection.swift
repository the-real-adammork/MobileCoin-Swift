...
class HttpConnection: ConnectionProtocol {
    ...
    func performCall<Call: Callable>(
        _ call: Call,
        completion: @escaping (Result<Call.Response, ConnectionError>) -> Void
    ) where Call.Request == () {
...
extension HttpConnection {
    private struct Inner {
        ...
        func urlRequestWithCallOptions() -> URLRequest {
            // TODO - FIX, need exact URL
            var request = URLRequest(url: url.httpBasedUrl)
            session.addRequestHeaders(to: &request)
            return request
        }
