...
public struct AccountKey {
    ...
    func subaddressSpendPrivateKey(index: Int) -> RistrettoPrivate? {
        let sspk = [subaddressSpendPrivateKey, changeSubaddressSpendPrivateKey]
        guard index < sspk.count else {
            return nil
        }
        return sspk[index]
    }
